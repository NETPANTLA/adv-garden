...
const nodes = [
  ...
  N('mislabeling','Mislabeling','effect','Conserva la detección del objeto, pero fuerza una clase equivocada.',['TOG','L_class','targeted']),
  N('untargeted','Untargeted','effect','Busca cualquier fallo del sistema sin fijar de antemano el resultado exacto.',['FGSM','PGD','C&W']),
  N('whitebox','White-box','access','El atacante conoce arquitectura, pesos y gradientes del modelo objetivo.',['acceso total','gradientes']),
  ...
  N('targeted','Targeted','access','Persigue un resultado específico: una clase, una identidad o una salida previamente elegida.',['objetivo específico']),
  ...
  N('physical','Realizable físicamente','cross','Perturbación diseñada para sobrevivir impresión, iluminación, distancia, ángulo y captura por cámara.',['mundo físico','robustez']),
  ...
  N('universal','Universal','pipeline','Un único patrón o perturbación se reutiliza en múltiples imágenes, personas o escenas.',['reutilizable'],[['Universal Adversarial Perturbations','https://arxiv.org/abs/1610.08401']]),
  ...
  N('dual','Dual-use','cross','La misma técnica puede proteger privacidad o facilitar evasión ilícita; el contexto define su marco político-jurídico.',['ética','política','contexto']),
  ...
  N('openvocab','Vocabulario abierto','vision','Grounding DINO, YOLO-World y YOLOE detectan objetos solicitados mediante texto.',['texto','Grounding DINO'],[['Grounding DINO','https://arxiv.org/abs/2303.05499']]),
  ...
  N('nir','RGB–NIR / nocturna','sensors','Combina captura visible e infrarrojo cercano. Un parche RGB no necesariamente transfiere bajo iluminación IR.',['infrarrojo','noche']),
  ...
  N('alpr','ALPR / LPR','sensors','Pipeline compuesto por detector de vehículo, detector de matrícula y OCR; debe evaluarse por etapas.',['matrículas','OCR']),
  ...
  N('multimodal','Ataque multimodal','cross','Combina superficies visuales, acústicas o temporales, relevante para cámaras corporales y vigilancia conectada.',['audio','video','tracking']),
  ...
  N('advhat','AdvHat / gafas','vision','Artefactos faciales físicos que buscan modificar detección o embeddings de identidad.',['rostro','físico'],[['Paper AdvHat','https://arxiv.org/abs/1908.08705']]),
  ...
  N('objecthider','Object Hider','effect','Parche físico orientado a ocultar objetos frente a detectores mediante objetivos basados en mapas de activación o consenso.',['vanishing','patch'],[['Paper Object Hider','https://arxiv.org/abs/2010.14974'],['Repositorio','https://github.com/FenHua/DetDak']]),
...
// Cuatro capas de lectura. Los nodos no dependen de su posición visual para expresar jerarquía.
...
function showDetail(n){
  ...
  const sourceBlock=(direct.length||relatedSources.length)?`<div class="detail-block" style="--node-color:${f.color}"><h3>${direct.length?'FUENTES DIRECTAS':'FUENTES RELACIONADAS'}</h3>${[...direct,...relatedSources].map(s=>`<a class="source-link" href="${s[1]}" target="_blank" rel="noreferrer">${s[0]}</a>`).join('')}</div>`:'';
